...
const MenuEdit: React.FC = () => {
    ...
    const { data, loading, error } = useFetch<Menu>(`${process.env.REACT_APP_SERVER_URL}/menu/getmenu/${id}`);
    const { update: saveMenuData, loading: saving, error: saveError } = useUpdate<Menu>(`${process.env.REACT_APP_SERVER_URL}/menu/updatemenu`);
    ...
    return (
        <div className="container mx-auto p-6">
            ...
            {saveError && <p className="text-red-600">{saveError.message}</p>}
        </div>
    );
};
